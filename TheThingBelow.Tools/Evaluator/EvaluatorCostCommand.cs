...
public static class EvaluatorCostCommand
{
    ...
    public static Simulation CopyOf(Simulation run, ulong seed, BattleContent content)
    {
        ...
        return Simulation.Resume(seed, RunSnapshotText.Read(ref reader), CostFight.Map(), content, run.State.Notices, run.State.Story.Content, DebugIntentHandlers.None);
    }

    private static Simulation IntoFight(ulong seed, BattleContent content)
    {
        Simulation run = Simulation.Start(seed, CostFight.Map(), content, CostFight.Notices(), CostFight.Story(content), DebugIntentHandlers.None);
