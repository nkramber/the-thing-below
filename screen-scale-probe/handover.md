...
2. The system reports a screen scale of 2 on that screen, and the frame fits at 3x.
3. A windowed run reports 1280 by 720 alone, so exit test 4 fails for a windowed run.
4. The dialogue box holds 156 characters at the UI scale of 1x, and 76 characters at 2x.
5. Thus the dialogue limit of 80 characters in the `game-text-style` skill holds at 1x alone.
6. The Windows export templates of the Deck test cover each platform, so the Windows build
   needed no new download.
7. The probe measures the line box of 16 pixels, and not the cap height of a glyph. The line
   box gives a larger number than the glyph number of F-67.
8. The REF 2 rule of `ste-check` reads a bare file name as one file of the checkout. This spike
   adds a second `project.godot`, so that name matched two files, and the pre-commit hook
   refused the commit. The owner chose the exact citation on 2026-09-18. This branch now cites
   `TheThingBelow.Game/project.godot` in `.claude/skills/csharp-conventions/SKILL.md` and in
   `docs/roadmaps/phase-1-foundations.md`. A later PR that adds a second Godot project meets
   the same result, and the same fix serves it.
...
- Traps: the seven findings above, and the trap list of `readme.md`.
...
7. The exact citation of finding 8 in the two files above, because `main` still holds the bare
   name. The fix stays correct with one Godot project, and it is necessary with two.
