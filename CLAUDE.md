...
- Every PR and every commit answer the pass (D-66, D-944). After an approval, a skip-set commit with no settings file takes no new review (D-943, D-1122). The `review-override` label exempts a documentation PR from the Codex review alone, and only when the PR changes no row of `docs/decisions.md` (D-401).
- The override set holds `docs/`, `README.md`, `CLAUDE.md`, `AGENTS.md`, `.claude/` without its settings files, and `.github/pull_request_template.md` (D-16, D-71, D-239, D-700, D-1086). Every other path takes the review, `.github/workflows/` and `content/` included (D-185, D-560).
