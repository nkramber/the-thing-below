...
public sealed class LightContentTests
{
    ...
    {
        // D-846, D-913: the glow of a fire names a key of the one palette, as its light does.
        List<ContentFile> files = LightFixtures.Files(LightFixtures.DecorBody(string.Empty), LightFixtures.SetupBody());
    ...
    {
        // T-2: the glow file is one file of every build, and its absence is an error.
        List<ContentFile> files = LightFixtures.Files(LightFixtures.DecorBody(string.Empty), LightFixtures.SetupBody());
