...
public sealed class CrashTextTests
{
    ...
    public void TheRecordOfACrashFileReachesTheStateHashOfTheRun()
    {
        ...
        RunState replayed = RunReplay.Play(read.Record!, SaveRuns.ContentHash, TestMaps.Room, TestBattles.Content, DebugIntentHandlers.None);
    ...
    {
        SimulationException fault = Assert.Throws<SimulationException>(
            () => Simulation.Start(Seed, TestMaps.Room, TestBattles.Content, DebugIntentHandlers.None).Step([Intent.OfPlayer(IntentIds.CloseMenu)]));
