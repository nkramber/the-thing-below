...
2026-09-21 light pass: the budget test counts the worst view of the Deck (D-842). Each torch lights itself from its decor kind, and a decor file beside each map places it (D-843, D-844). Walls cast full-tile shadows, and each light names a palette key with a strength (D-845, D-846). The party carries a light, and PR-91 makes the torch an item (D-847, D-848). The art target is the HD-2D look in 2D, and PR-92 adds three full-screen passes after a new Deck sweep (D-849). A battle takes the ambient light and one key light of its map (D-850).
...
2026-09-23 review loop pass: the owner reads a summary in four sections inside the merge question (D-942). After an approval, a commit of documents alone keeps the `review-gate` check green with no new review (D-943). That commit still gets its Gitar pass (D-944).
...
| Steamworks glue | Game | Steam client | the start of Steamworks | Low. Steam builds alone, with no cloud code (D-460, D-461). PR-78 adds the binding and the start, with no controller type call, and PR-40 sets Auto-Cloud (D-553, D-815) |
...
- M-6: the Deck frame time on the first playable, against 60 frames per second (D-161). The readability of the body font and the 32-pixel sprites at the scale that D-633 and D-707 set (D-92, D-228, D-621).
...
| F-47 | D-188 reads HDR 2D as the way to glow light alone and keep sprites and tiles clean. The canvas shaders add each light with no upper clamp, and HDR 2D keeps values above 1.0. The glow threshold starts at 1.0, and in SDR it "needs to be decreased below 1.0 when using glow in 2D". So a bright light on a pale sprite can glow, and in SDR a bright art pixel can glow too. The Compatibility renderer of the screen tests "uses a different implementation" of glow. Sources: the external facts of `docs/roadmaps/area-effects.md`, read 2026-09-15 | 2026-09-15 | ✅ PR-59: HDR 2D with a threshold above the brightest lit art, which a load test holds (D-910, D-915) |
...
| F-96 | The paired sweep of 2026-09-21 on the OLED Deck held each stage. Under Mobile, 24 paired lights gave 4.17 ms at the 95th percentile, and 24 paired lights with 2048 particles, glow, fog, and a transition gave 4.55 ms, with a worst frame of 11.63 ms and no frame over 16.667 ms. Forward+ gave 4.55 ms and 5.00 ms. The scene is the stress scene of `spike/deck-test`, which lights the full 1280 by 720 frame, and the game lights a world of 640 by 360 (D-634). The reports lie in the folder of 2026-09-21 under the results of that branch | 2026-09-21 | ✅ PR-56: the light row of 24 stands (D-854, G-14). M-6 still measures the game itself (D-523) |
| F-97 | Each stream of PR-58 rode the view, and never the world. The weather and the embers of each torch stood still on the screen while the map moved under them. A Godot particle node with `local_coords` off holds each live particle in the space of the viewport, so the node moved the start box and the engine kept each particle on the screen. The owner found the fault in a play session | 2026-09-22 | ✅ PR-58: each stream keeps `local_coords` on, and it moves its start box with `emission_shape_offset`. The node stands still, so a particle that lives already stays in the world. The fog moves its region with the view for the same reason |
...
| F-103 | A probe of PR-59 on 2026-09-23 drew a view of HDR 2D under Mobile on the Mac. The view keeps linear values above 1.0, and a flame at 12 read 12.1. The glow of Godot works in a `SubViewport` with its own 3D world and the canvas background. Lit art at 3.4 with a threshold of 4 gave no glow. The texture of an HDR view reaches a view with no HDR 2D in linear values, with no conversion, so the world drew too dark. Godot also reads a modulate color as sRGB, and it converts that color to linear | 2026-09-23 | ✅ PR-59: the world view turns linear light into sRGB in a shader at its draw into the frame, and each glow rectangle turns its light into sRGB before it reaches a modulate (D-910, D-915) |
...
27. **G-27.** Every effect draws with the palette of 64 colors and hard edges, and no smooth gradient. The rule covers each particle and each transition (D-181, D-622). The fog alone fades in 2 to 8 steps over blocks of art pixels (D-900, D-907). The glow blooms smoothly, and its colors can leave the palette (D-911, D-915). The three passes of PR-92 take a smooth mode and a stepped mode until the owner picks one (D-917). The owner reads each new effect as its PR lands (D-623).
...
Phase file: `docs/roadmaps/phase-2-first-playable.md`. This is the largest phase: 52 PRs, and 49 of them land before Gate 2. Each system, each tool, and each group of screens takes an id of its own (D-486, G-8).
...
26. PR-11: the evaluator, the enemy profiles, and the groups, with the cost of a turn (D-65, D-534, F-53).
27. PR-67: the character level, the experience, MP, and the stat curves (D-34, D-42, D-536, D-537).
28. PR-62: the menu windows, the party and status windows, the dungeon map screen, and the notices (D-211, D-558, D-567, D-569).
29. PR-68: the story scene format and runner, the join step, the flags, and the conditions, before PR-12 (D-541, D-544, D-556, D-563).
30. PR-50: the screenplay tool, right after PR-68 (D-173, D-545).
31. PR-12: the lessons, the slots, the forms, and the aptitudes (D-272, D-356, D-539).
32. PR-13: the six gear slots, the items, and the pack (D-44, D-382).
33. PR-91: the torch item, right after PR-13 (D-847, D-848).
34. PR-14: the hub map, the NPCs, the rest, the save, and the party and lesson swaps (D-59, D-112, D-356).
35. PR-65: the shop and the gold economy, after PR-13 (D-60, D-530).
36. PR-36: the dialogue box, the portraits, and the story scene on screen (D-114, D-223).
37. PR-15: the headless runner, the two bot policies, and the bot job (D-64, D-505).
38. PR-49: the night job and the `night-gate` command, right after PR-15 (D-496, D-507).
39. Owner: require the bot and `night-gate` checks on `main` after their first runs.
40. PR-16: the treasure, the doors, the keys, and the save points (D-41, D-555).
41. PR-64: the traps, the hazards, and the statuses that last on the map (D-390, D-529).
42. PR-35: the region map of nodes and routes (D-113).
43. PR-38: the synthesizer, the two note formats, the render hashes, and the `listen` command (D-432, D-438).
44. PR-69: the audio player, the four buses, and the mute (D-435, D-546).
45. PR-70: every rule of what plays when (D-413, D-546).
46. PR-71: the sound room in a development build (D-439, D-546).
47. PR-51: the PNG import for a hand edit (D-107, D-497).
48. PR-52: the map preview as a PNG (D-165, D-497).
49. PR-53: the tile-edge tool and the edge files (D-204, D-501).
50. PR-72: the music, the themes, and the sounds of the first playable (D-549).
51. PR-17: the village, the mining town, and the hanging cells as content (D-362, D-369, D-370).
52. M-3: the wall time of each leg, and the crash and softlock counts of seven nights (D-507, D-509).
53. M-4: the turns of each encounter and the party downs of each dungeon, by policy.
54. M-6: the frame time and the readability on the Deck, at the scale of OQ-183 (D-161, D-621).
55. Owner: set the M-4 band from the M-4 numbers, before the sign-off (D-571).
56. **← GATE 2 (first playable).**
57. PR-74: the capture, which replays a record into frames and audio (D-476, D-551).
58. PR-75: the store text and the checklist of the owner steps (D-452, D-550).
59. PR-76: the store art and the five screenshots (D-475, D-550).
60. Owner: pay the Steam Direct fee, and put the store page public as Coming Soon (D-471).
...
4. PR-78: the Steamworks binding and the start (D-460, D-553, D-815).
...
16. PR-48, PR-56, PR-93, PR-63, PR-57, PR-58, PR-94, PR-59, PR-92, PR-95, PR-60, PR-96.
