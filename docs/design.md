...
| F-47 | D-188 reads HDR 2D as the way to glow light alone and keep sprites and tiles clean. The canvas shaders add each light with no upper clamp, and HDR 2D keeps values above 1.0. The glow threshold starts at 1.0, and in SDR it "needs to be decreased below 1.0 when using glow in 2D". So a bright light on a pale sprite can glow, and in SDR a bright art pixel can glow too. The Compatibility renderer of the screen tests "uses a different implementation" of glow. Sources: the external facts of `docs/roadmaps/area-effects.md`, read 2026-09-15 | 2026-09-15 | ✅ PR-59: the glow pass of our own reads the light sources alone, so no lit art can glow (D-913). Round 1 held a threshold above the brightest lit art (D-910) |
...
| F-103 | A probe of PR-59 on 2026-09-23 drew a view of HDR 2D under Mobile on the Mac. The view keeps linear values above 1.0, and a flame at 12 read 12.1. The glow of Godot works in a `SubViewport` with its own 3D world and the canvas background. Lit art at 3.4 with a threshold of 4 gave no glow. The texture of an HDR view reaches a view with no HDR 2D in linear values, with no conversion, so the world drew too dark. Godot also reads a modulate color as sRGB, and it converts that color to linear | 2026-09-23 | ✅ PR-59 round 1 turned the linear light of the world into sRGB in a shader. Round 2 keeps the world in SDR, so it needs no conversion (D-913) |
| F-104 | Round 1 of PR-59 drew the world in HDR 2D, and Godot then blends each item in linear light. The light pools turned paler, and the shadows turned darker: a corner pixel of the map fixture went from 6,7,14 to 2,2,7. The fog turned thicker, and an enemy in fog lost contrast. The fog test of Core computes each gap in sRGB, so it passed while the screen showed less. The glow of Godot also reads the picture at half size first, so a flame particle of one art pixel gave no glow | 2026-09-23 | ✅ PR-59 round 2: the world stays in SDR, and the glow pass of our own reads a glow rectangle of each source (D-913) |
| F-105 | Three facts of Godot 4.7.2 that the glow pass of PR-59 met. A view draws a canvas item only when the item and each parent share a layer with the cull mask of the view. A texture rect holds the size of its texture until its expand mode changes, so an object initializer that sets the size first keeps the larger size. The shader compiler refuses `TEXTURE` as the argument of a function, and it writes the error to the log alone | 2026-09-23 | ✅ PR-59: the world of a screen takes the glow layer too, the expand mode comes before the size, and the blur reads the texture inside `fragment()` (D-913) |
...
27. **G-27.** Every effect draws with the palette of 64 colors and hard edges, and no smooth gradient. The rule covers each particle and each transition (D-181, D-622). The fog alone fades in 2 to 8 steps over blocks of art pixels (D-900, D-907). The glow blooms smoothly, or it fades in steps over blocks, and its colors can leave the palette (D-911, D-914). The owner reads each new effect as its PR lands (D-623).
...
21. PR-59: the glow on light sources alone, first on the fire of each wall torch (D-188, D-910 to D-914).
