...
| F-64 | A headless Godot session whose managed assembly does not load never reaches `Quit`, and it runs without end. The session writes `Cannot instantiate C# script` and then waits. With `--quit-after` the same session ends with an exit code of 0 and no success line, so the exit code hides the fault. The `smoke` target of the Makefile read no log, so it reported success on a session that never ran the boot code | 2026-09-16 | ✅ PR-1: the smoke session of the Makefile and of CI runs with `--quit-after 600`, and each one fails when the success line is absent. A broken session now fails in about 6 seconds, and not at the time limit of the job (T-2) |
...
19. **G-19.** Every screen designs to one 16:9 frame of 1280 by 720 with 32-pixel tiles (D-568). The Steam Deck at 1x with the CRT on is the floor, with black bars above and below (D-92, D-120, D-228). Every other screen shape shows black bars too. A desktop at 1920 by 1080 must look good, and the fit of D-232 holds that rule (D-568).
...
36. PR-37: the CRT shader and its toggle (D-105, D-120).
...
48. M-6: the frame time and the readability on the Deck (D-161, F-18).
