...
- The command passes a PR in the override set with the `review-override` label that changes no decision row (D-16, D-71, D-239, D-401). A change to a row is a change to a line of a decision table (D-609). The command refuses the label on a PR that changes `.claude/settings.json` or .claude/settings.local.json (D-700, D-1085).
