...
- The command passes a PR in the override set with the `review-override` label that changes no decision row (D-16, D-71, D-239, D-401). A change to a row is a change to a line of a decision table (D-609). The command refuses the label on a PR that changes `.claude/settings.json` or .claude/settings.local.json (D-700, D-1086).
...
- The batch holds each story scene that the PR changes, against the base content (D-1015). The tool writes a marked section of a PR body file, and it refuses a body above the limit of GitHub (D-1016).
- Each step starts with its number, and each step that is not a line gives one action line (D-1017).
- PR-70 adds the look-up of each cue in the audio file, because a story scene names no cue (D-548, D-1014).
