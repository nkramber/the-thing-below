...
| F-47 | A bright light on a pale sprite can pass the glow threshold | PR-59: the glow pass of our own reads the light sources alone (D-913) |
...
| Glow | A soft glow on light sources alone, over the world view | No | D-188, D-913 |
...
- PR-56 adds the budget file and its test with the rows for light. PR-57 adds the particle row, and PR-58, PR-59, and PR-60 add their full-screen passes. PR-94 counts one pass for each fog (D-898). PR-59 counts one glow pass on every map and every fight (D-913).
...
- UI never glows (D-210). The pass adds the glow over the world view and under the UI layer of the frame.
- PR-59 holds two glows, and the owner keeps one (OQ-232). This section holds round 2, the glow pass of our own (D-913). Round 1 drew the world in HDR 2D with the glow of Godot (D-910).
- Each fire holds a glow: a small rectangle of one palette key, with a strength, a size, and a place from the light (D-913). A strength of 0 gives no glow.
- The rectangle draws on a glow layer alone. The world view never draws that layer, and a mask view that shares the world draws it alone.
- A view draws an item only when each parent shares a layer with the view (F-105). Thus the world of a screen takes the glow layer too.
- No sprite, tile, or UI node draws on the glow layer, so none can glow, whatever its light (F-47).
- A quarter view blurs the mask. Each cell reads the exact mean of its 4 by 4 art pixels, so the glow never shimmers when the view moves by one pixel.
- The frame adds the blur over the world. The glow file holds the intensity, the mode, and the pulse, in integers (D-517).
- The smooth mode reads the blur between its cells (D-911). The stepped mode fades in 2 to 8 steps over blocks of art pixels, as the fog does (D-907, D-914).
- Each glow pulses on a slow cosine of the tick, with a phase from the id of its source (D-913). The torch light keeps its steps (D-891), and the glow never flickers.
- The world stays in SDR, so the light, the fog, and the fog test keep their approved look (F-104).
- The glow is one full-screen pass on every map and every fight, so the effect budget counts it with the fog (D-523).

> *In plain English:* flames give off a soft haze of light that swells and fades, and the people and walls that they light stay crisp. Only a small glow patch at each flame feeds the haze, so no lit wall or figure can glow.
...
- The glow of D-188 blooms by design, outside the palette (D-911). A glow layer that no sprite or tile draws on keeps it off them (D-913, F-47).
...
| PR-59 | The glow on light sources alone, with two glows to compare | D-188, D-910 to D-914 |
...
- PR-59 gives the glow on light alone, as a smooth bloom (D-188, D-911, D-913).
...
- OQ-102: how glow stays off sprites. Resolved 2026-09-23 by D-910 to D-914.
- OQ-232: the glow that stays. Blocks PR-59.
