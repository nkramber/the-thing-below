...
### 7.37 PR-102: the gate fixes of the repository review

Area file: `area-ci.md` section 7.24.

**Scope.** The owner put seven findings of the repository review of 2026-09-24 in one PR (D-1080):

- Each git list of paths in a workflow takes `--no-renames` (F-108).
- The review gate gives each value that grows with the PR to jq through a file (F-109).
- Each push to `main` gets a concurrency group of its own commit (F-110).
- The import and export steps keep the exit code and print the log first (F-111).
- AGENTS 1 compares each byte, and DOCS 1 reads the exact form of each row (F-112).
- The review gate refuses the label on .claude/settings.local.json, and git ignores the file (D-1085, F-113).
- Each program of the `codex-review` command runs under a time limit (D-1086, F-114).
- The owner answers on four other findings of the review (D-1081 to D-1084).

**Out of scope.**

- The input fixes of D-1082 and D-1083, and the grace time of D-1084. Each one changes code that PR #78 changes, so each lands after it.
- A change of the setup of the `codex-review` command (D-1081).
...
- No limit of D-1086 stops a normal review.

**Questions.** None. D-1080 to D-1086 hold the answers of 2026-09-24.
...
### 7.47 PR-38: the audio synthesizer and the first sounds
...
### 7.48 PR-69: the audio player base
...
### 7.49 PR-70: the rules of what plays when
...
### 7.54 PR-72: the music and the sounds of the first playable
...
7. PR-12, PR-13, PR-99, PR-91, PR-100, PR-102, PR-14, PR-65: the build of a party, the stat set, the torch, the end of the Gitar pause, the gate fixes of the repository review, the hub, and the shop.
