...
- The lesson file, with the name, the aptitude kind, and the forms of each lesson. Each form names an ability id of PR-80 (D-785, D-1026).
- The lesson slots on the character: two at level 1, and one more at each level of the rules file (D-356, D-1018).
- The swap of lessons at a swap place alone, which holds for the dungeon visit. PR-14 and PR-16 mark the places, and a debug command marks one (D-356, D-1030).
...
- The named forms of each lesson, and the point total that opens each form. A character keeps each opened form (D-539, D-1027).
- The cure and the boon effects of the ability file (D-1029).
...
- The eight kinds, the main aptitude of each character, and the bonus of a lesson of that kind, in basis points (D-274, D-281, D-358, D-791, D-1028).
- The half bonus of a side aptitude, which reads the same table (D-360, D-791).
- The side aptitude behind a story flag of PR-68, with an empty mark in the lesson window before the unlock (D-282, D-283, D-538, D-556, D-1033).
...
- The flash of each spell, with a point light of PR-56 for its length, its effect files, its reduced forms, and its captures (D-183, D-186, D-863, D-878, D-1032).
- The refusal of a rite of a silenced holder, in a fight and from the menu (D-393, D-806).
- The lesson window in the stack of PR-62, and the Lessons command of a battle with its list of forms (D-1027, D-1031).
...
### 7.38 PR-102: the gate fixes of the repository review

Area file: `area-ci.md` section 7.24.

**Scope.** The owner put seven findings of the repository review of 2026-09-24 in one PR (D-1081):

- Each git list of paths in a workflow takes `--no-renames` (F-109).
- The review gate gives each value that grows with the PR to jq through a file (F-110).
- Each push to `main` gets a concurrency group of its own commit (F-111).
- The import and export steps keep the exit code and print the log first (F-112).
- AGENTS 1 compares each byte, and DOCS 1 reads the exact form of each row (F-113).
- The review gate refuses the label on .claude/settings.local.json, and git ignores the file (D-1086, F-114).
- Each program of the `codex-review` command runs under a time limit (D-1087, F-115).
- The owner answers on four other findings of the review (D-1082 to D-1085).

**Out of scope.**

- The input fixes of D-1083 and D-1084, and the grace time of D-1085. Each one changes code that PR-101 changes, so each lands after it.
- A change of the setup of the `codex-review` command (D-1082).

**Exit tests.**

1. `WorkflowPathListTests` finds each git list of paths in the workflows, and each one takes `--no-renames`.
2. The facts step of the review gate gives each value that grows with the PR to jq through a file.
3. `EachPushToMainTakesAGroupOfItsOwnCommit` and `EachEditorStepKeepsItsCodeAndPrintsItsLogFirst` pass.
4. AGENTS 1 finds a difference of a line end alone, and DOCS 1 finds a row with a partial backtick.
5. A labeled PR that changes .claude/settings.local.json faults RG 1.
6. A program past its limit stops with a fault that names it, and each child of it stops too.

**Review focus.**

- The facts file of the review gate holds the same values as before for a normal PR.
- No limit of D-1087 stops a normal review.

**Questions.** None. D-1081 to D-1088 hold the answers of 2026-09-25.
...
### 7.40 PR-104: the boot message, the enemy reply, and six rule guards of the repository review

Area files: `area-ui-input.md` section 7.14, `area-exploration.md` section 7.14, and `area-battle.md`.

**Scope.** The owner put eight findings of the repository review of 2026-09-24 in one PR (D-1098):

- A settings file that fails to load goes aside, and the start runs on the defaults with a message (D-1099, D-1100, F-131).
- A crash before the screen shows its message on a frame of the default display (F-131).
- The reply of the evaluator scores the best legal strike of the next character (D-1101, F-132).
- The enemy phase has a bound, and no fight starts or resumes with no character on its feet. An absorbed hit rolls no status (D-1105, F-133).
- The crash message names its folder with no account name (D-1102, F-134).
- The load and the resume refuse six states that no rule makes (D-1107, F-135).
- A tile trigger plays before a step into an enemy on the same tick (D-1103, F-136).
- A step into the enemy whose mark runs takes the side of the beat (D-1104, F-137).
- An absorb at full health shows no heal of 0 (D-1106, F-138).

G-17 now states that a reader of Core counts (D-1108).

**Out of scope.**

- The content drift of saves (P2-3) and the trust of the review gate (P3-9). Each one takes design questions of its own first.
- A field of the settings file for a value outside its range. The message then names the whole file, and the log line holds the error.

**Exit tests.**

1. `AnEnemyPhaseThatReachesNoCharacterFailsWithTheGroupAndNeverHangs` and `AStepIntoAGroupWithEveryCharacterDownFailsWithTheGroup` pass.
2. `AnAbsorbedHitGivesNoStatusAndAHitOfTheSameMoveDoes` passes.
3. `AStepBehindAnAllyEarnsNoRowCreditAgainstAStrikeOfAnyReach` and `TheReplyTakesALessonStrikeThatOutDamagesTheBasicAttack` pass.
4. The `evaluator-cost` command reports a turn below 1 ms at the 95th percentile on the Steam Deck, with lessons (D-961).
5. `AnArrivalOnATriggerTilePlaysItsSceneBeforeTheStepIntoTheGuardBesideIt` passes.
6. `AStepIntoTheEnemyWhoseMarkRunsTakesTheSideOfTheBeat` passes.
7. `ARulesFileThatBreaksARuleFailsWithTheField` refuses a ceiling of 6667, and `TheRulesTakeTheMissCeilingAndTheSlotCountAtTheirBounds` passes.
8. `ARefusedFileIsKeptAsideAndReplacesAnOlderKeptFile` and each case of `SettingsFallbackTests` pass.
9. `TheShownFolderOfEachSystemNamesNoAccount` and `AnAbsorbAtFullHealthShowsNoNumberAndAnyOtherAbsorbShowsItsHeal` pass.

**Review focus.**

- The simulation version rises to 28, and the identity file changes with it (G-17, D-504).
- The new player strings are the two absorb lines, the folder line, and the five lines of the settings message (D-57).

**Questions.** None. D-1098 to D-1108 hold the answers.

> *In plain English:* a broken settings file stopped the game with no word. Now the game keeps the file aside, says so, and starts. Enemies now fear a caster's spell, and six odd states fail with a clear error.

### 7.41 PR-105: the save drift rules, the wipe pick, the save hook, and six guards of the repository review

Area files: `area-core.md` sections 7.9 to 7.11, `area-exploration.md`, `area-ui-input.md`, `area-battle.md`, and `area-ci.md` section 7.25.

**Scope.** The owner put nine findings of the repository review of 2026-09-24 in one PR (D-1109):

- A save of another build matches each enemy by its id, and it finds a moved story step by its id (D-1111, D-1112, F-139).
- A lead off an edited map moves to the spawn point (D-1111).
- A change of a party rule refuses a save until the migration of that patch reads it (D-1110, F-139).
- A test holds a digest of those rules (D-1110).
- Each change of a save of another build writes a warning line of the log (D-1113).
- A wipe reloads a save of its own run alone, and smoke reads a new empty save folder (D-1114, F-140).
- `GameRun.Save` bounds the run record (D-1115, F-141).
- The coverage job checks the SHA-512 of its package (D-1116, F-142).
- A steal checks its remainder, and each damage cut checks its overflow (F-143).
- det-lint refuses a file or OS call, disabled text, a scene value across lines, and a binary scene (D-1117, F-144).
- A test reads the drawing of each patrol, the lead, and each tile kind, and smoke draws a lit copy of the map (D-1118, F-145).
- The remap screen marks each conflict, and its line names the conflict of the row (D-1119, F-146).
- Stale marks of the design, rows of section 9, and comments of the code match the merged work.

**Out of scope.**

- The trust of the review gate (P3-9), and the key names of the remap screen (P3-24).
- The flake of the screen test (P3-25), and the tests of the crash path (P3-26).
- The performance measurements (P3-36) and the shimmer of a paused step (P3-37).
- The first migration of a party rule, which PR-30 writes with its retune (D-1110).

**Exit tests.**

1. `AnotherBuildFindsAStepThatAnInsertedStepMovedAndLogsIt` and `AStepIdThatTheStorySceneNoLongerHoldsRefusesTheSave` pass.
2. `AnotherBuildPlacesANewEnemyOnItsStationAndLogsIt` and `ThisBuildStillRefusesASaveWithAnotherEnemyCount` pass.
3. `ThePartyRuleNumbersThatASaveReadsMatchTheirDigest` passes, and `AnotherBuildStillRefusesAPartyThatAChangedLevelTableBreaks` passes.
4. `AWipeNeverReloadsASaveOfAnotherRun` and `AReloadDropsASaveOfAnotherRunAndStartsTheRunAgain` pass.
5. `ASaveDropsTheIntentsBeforeItSoTheRecordOfAHeldWalkStaysBounded` passes.
...
- The simulation version rises to 29, and the identity file changes with it (G-17, D-504).
- Save format 14 and record format 4 hold the step id (D-1112).
- The one new player string is the line of a conflict with its place (D-57).

**Questions.** None. D-1109 to D-1119 hold the answers.
...
### 7.42 PR-14: the hub map, the NPCs, and the services
...
### 7.43 PR-65: the shop and the gold
...
### 7.44 PR-36: the dialogue box, the portraits, and the story scene on screen
...
### 7.45 PR-15: the headless runner and the bots
...
### 7.46 PR-49: the night job and the night gate
...
### 7.47 PR-16: the dungeon parts, the death, and the save points
...
### 7.48 PR-64: the traps, the hazards, and the statuses on the map
...
### 7.55 PR-38: the audio synthesizer and the first sounds
...
### 7.56 PR-69: the audio player base
...
### 7.57 PR-70: the rules of what plays when
...
### 7.59 PR-72: the music and the sounds of the first playable
...
### 7.60 M-3, M-4, and M-6: the measurements of the phase
...
### 7.61 Gate 2: the first playable
...
### 7.63 PR-75: the store text and the owner steps
...
### 7.64 PR-76: the store art and the screenshots
...
7. PR-12, PR-13, PR-99, PR-91, PR-100, PR-101, PR-102, PR-103, PR-104, PR-105, PR-14, PR-65: the build of a party, the stat set, the torch, the end of the Gitar pause, the torch and pad fixes, the gate fixes, the input fixes, the boot and rule fixes, and the save and guard fixes of the repository review, the hub, and the shop.
