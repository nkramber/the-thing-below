...
The design doc holds the system map (section 3), the cost model (section 4), and the guardrails (section 6). The file `area-tools.md` holds the synthesizer and the listen command, and `area-ci.md` holds the job that renders and checks each hash. The file `area-core.md` holds the content reader and the events that Core emits. The file `area-story.md` holds the scene that a cue serves, and `area-battle.md` the fight that makes each sound. The file `area-exploration.md` holds the maps, and `area-ui-input.md` the settings screen and the menu sounds.
...
### 7.6 Scenes, the menus, and the main theme

Built by PR-70. Phase file: `phase-2-first-playable.md`.

- A scene keeps the place music unless a cue serves it (D-418, D-548). A cue comes from a small set of mood tracks, such as tension, grief, and menace.
- A few key scenes have a track of their own (D-418). The file `area-story.md` holds the scene.
- The title screen and the last scene of region one play the main theme (D-427). The region map has a track of its own.
...
> *In plain English:* a short tune stands for each group and each person, and other music quotes it. Scenes borrow a mood track when the place music does not fit.
...
- An audio file names the content ids that it serves: the maps, the battles, the menus, and the scenes (D-548).
- A rule file never names a track, a sting, an ambience, or a sound effect, as D-519 asks for art and D-495 for effects.
- The cue of a scene lives in the audio file, keyed by the scene and the step (D-548). That revises D-418 in part.
...
| The scenes that a cue serves | `area-story.md` | PR-68 and PR-36 |
